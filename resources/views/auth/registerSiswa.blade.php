...
<div class="card m-auto mt-5 mb-2 rounded-4 col-11 col-lg-4">
    <form class="p-4" action="/register/siswa" method="POST">
      @csrf
      ...
      <div class="form-floating mb-3">
        <input type="text" name="nisn" id="nisn" placeholder="nisn" class="form-control @error('nisn') is-invalid @enderror" value="{{ old('nisn') }}">
      ...
      <div class="mb-3">
        <select class="form-select" name="jurusan" aria-label="Default select example">
            ...
            <option value="{{ $kl->kode_jurusan }}">{{ $kl->nama_jurusan }}</option>
            @endforeach
          </select>
        @error('jurusan')
