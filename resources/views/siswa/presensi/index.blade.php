    ...
    <div class="mt-5 mb-5">
        <table class="table table-responsive">
            ...
            <tr>
                ...
                <td>:</td>
                <td><strong>{{ Auth::user()->nisn }}</strong></td>
            </tr>
            <tr>
            ...
            <tr>
                ...
                <td>:</td>
                @foreach ($user as $u)
                <td><strong>{{ $u->nama_jurusan }}</strong></td>
                @endforeach
            </tr>
        </table>
    </div>
                    ...
                    @foreach ($history as $hs)
                        @if ($hs->tanggal == $tanggal)
                            <tr class="text-center">
                                ...
                                <td>{{ $hs->nama }}</td>
                                <td>{{ $hs->tanggal }}</td>
                                <td>{{ $hs->jam_presensi }}</td>
                                <td>{{ $hs->nama_kelas }}</td>
                                <td>{{ $hs->nama_mata_pelajaran }}</td>

                                <td>
                                    <a href="/presensi_siswa/{{ $hs->id_presensi }}/edit" class="btn btn-info border-0">Isi Presensi</a>
                                </td>
                            </tr>
                            <?php $i++ ?>
                        @endif
