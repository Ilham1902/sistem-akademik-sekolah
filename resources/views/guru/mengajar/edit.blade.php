...
@section('body')
    ...
    <div class="card rounded-4 col-lg-12 mb-4 mt-4 p-2 shadow-lg">
        ...
        <div class="card-header">
            <a href="/mengajar" class="btn btn-secondary"><i class="bi bi-arrow-left"></i> Kembali</a>
        </div>

        <div class="card-body mt-3">

            @foreach ($history as $hs)
            <form action="/mengajar/{{ $hs->id_absen }}" method="post">
                @method('put')
                @csrf
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="nama_guru" class="form-control @error('nama_guru') is-invalid @enderror" id="nama_guru" placeholder="Agus Trisusillo, S.Kom" value="{{ old('nama_guru', $hs->nama_guru)}}" disabled>
                    @error('nama_guru')
                        <div class="invalid-feedback">
                            {{ $message }}
                        </div>
